...
<div class="main_content">
  ...
  @elseif(($_GET['c_filter']!='true') && ($_GET['con_filter']=='true') && ($_GET['y_filter']!='true'))
    <br><br>List of <b>{{$_GET['con_status']}} {{$_GET['business_type']}}</b> Contracts
  @elseif(($_GET['c_filter']!='true') && ($_GET['con_filter']=='true') && ($_GET['y_filter']=='true'))
    <br><br>List of <b>{{$_GET['con_status']}} {{$_GET['business_type']}}</b> Contracts whose Lease <b>{{ucfirst(strtolower($_GET['lease']))}}</b> Year is <b>{{$_GET['year']}}</b>
  @elseif(($_GET['c_filter']=='true') && ($_GET['con_filter']!='true') && ($_GET['y_filter']!='true'))
  ...
  @elseif(($_GET['c_filter']=='true') && ($_GET['con_filter']=='true') && ($_GET['y_filter']!='true'))
    <br><br>List of <b>{{$_GET['con_status']}} {{$_GET['c_name']}} {{$_GET['business_type']}} </b> Contracts
  @elseif(($_GET['c_filter']=='true') && ($_GET['con_filter']=='true') && ($_GET['y_filter']=='true'))
    <br><br>List of <b>{{$_GET['con_status']}} {{$_GET['c_name']}} {{$_GET['business_type']}} </b> Contracts whose Lease <b>{{ucfirst(strtolower($_GET['lease']))}}</b> Year is <b>{{$_GET['year']}}</b>
  @endif
</h4></center>
  <hr>
          ...
          <tr>
            ...
            @if($var->academic_dependence=="Yes")
              @if(empty($var->academic_season)&&empty($var->vacation_season))
                <td style="text-align: right;">{{$var->academic_season}}</td>
                <td style="text-align: right;">{{$var->vacation_season}}</td>
              @else
                <td style="text-align: right;">{{number_format($var->academic_season)}}</td>
                <td style="text-align: right;">{{number_format($var->vacation_season)}}</td>
              @endif
            @else
              @if(empty($var->amount))
     ...
     <table class="hover table table-striped table-bordered" id="InsuranceTable">
        ...
        @foreach($contracts as $var)
          <tr>
            ...
            <td style="text-align: right;">{{$var->currency}} {{number_format($var->commission)}}</td>
            @if($_GET['con_filter']!='true')
              <td>
                @if($var->contract_status==0)
                  TERMINATED
...
function settitle(){
 ...
 elseif(($_GET['c_filter']!='true') && ($_GET['con_filter']=='true') && ($_GET['y_filter']=='true')){
  ...
  elseif(($_GET['c_filter']=='true') && ($_GET['con_filter']!='true') && ($_GET['y_filter']!='true')){
   echo 'List of '.$_GET['c_name'].' '.$_GET['business_type'].' Contracts';
  }
  elseif(($_GET['c_filter']=='true') && ($_GET['con_filter']!='true') && ($_GET['y_filter']=='true')){
...
var filter = '<?php echo $_GET['con_filter'];?>';
        ...
        buttons: [
            {   extend: 'pdfHtml5',
                ...
                customize: function ( doc ) {
                  ...
                  doc.content[2].table.widths = [22, '*', 60, 45, 60, 60, 100, 60, 60, 70];
        ...
        buttons: [
            {   extend: 'pdfHtml5',
                ...
                customize: function ( doc ) {
                  ...
                  doc.content[2].table.widths = [22, '*', 70, 80, 70, 70, 70, 80,80];
                  var rowCount = doc.content[2].table.body.length;
                      for (i = 1; i < rowCount; i++) {
                         doc.content[2].table.body[i][0]=i+'.';
                      doc.content[2].table.body[i][1].alignment = 'left';
        ...
        buttons: [
            {   extend: 'pdfHtml5',
                ...
                customize: function ( doc ) {
                  ...
                  doc.content[2].table.widths = [22, '*', 60, 45, 60, 60, 90, 50, 50, 50, 50];
                  var rowCount = doc.content[2].table.body.length;
                      for (i = 1; i < rowCount; i++) {
                         doc.content[2].table.body[i][0]=i+'.';
                      doc.content[2].table.body[i][1].alignment = 'left';
            ...
            {   extend: 'pdfHtml5',
                ...
                customize: function ( doc ) {
                  ...
                  var rowCount = doc.content[2].table.body.length;
                      ...
                      doc.content[2].table.body[i][7].alignment = 'right';
                     //doc.content[2].table.body[i][5].alignment = 'right';
                      doc.content[2].table.body[i][8].alignment = 'left';
                      doc.content[2].table.body[i][4].alignment = 'left';
                    };
        ...
        buttons: [
            {   extend: 'pdfHtml5',
                ...
                customize: function ( doc ) {
                  ...
                  doc.content[2].table.widths = [22, '*', 70, 70, 70, 70, 70, 70,70,60];
                  var rowCount = doc.content[2].table.body.length;
                      for (i = 1; i < rowCount; i++) {
                         doc.content[2].table.body[i][0]=i+'.';
                      doc.content[2].table.body[i][1].alignment = 'left';
