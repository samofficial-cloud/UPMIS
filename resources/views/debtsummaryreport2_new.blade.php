...
<div class="main_content">
  ...
  <div class="container" style="max-width: 90%;">
    <br><br>
    <div class="tab">
            <button class="tablinks" onclick="openContracts(event, 'tzs')" id="defaultOpen"><strong>Payments in TZS</strong></button>
            <button class="tablinks" onclick="openContracts(event, 'usd')"><strong>Payments in USD</strong></button>       
    </div>

    <div id="tzs" class="tabcontent" >
      <br>
    <center><h4>Payments Made in TZS</h4></center>
    <hr>
  <table class="hover table table-striped table-bordered" id="tzsTable">
 ...
 <div id="usd" class="tabcontent" style="max-width: 100%;">
   <?php 
$totals = 0;
$totals2 = 0;
 ?>
  <br>
      <center><h4>Payments Made in USD</h4></center>
  <hr>
  <table class="hover table table-striped table-bordered" id="usdTable">
      <thead class="thead-dark">
        <tr>
          <th scope="col" style="width: 41.6px;">SN</th>
          <th scope="col" >Major Industry</th>
          <th scope="col" style="width: 25%;">Revenue</th>
          <th scope="col" style="width: 25%;">Debt</th>
        </tr>
...
return '\n\n Payment Made in USD';
     ...
     var table = $('#tzsTable').DataTable({
      ...
      "footerCallback": function ( row, data, start, end, display ) {
            ...
            var NumRows = $('#tzsTable').DataTable().rows().count();
            var i, pageTotal = 0,pageTotalUsd = 0, pageTotal2 = 0, pageTotalUsd2 = 0;

            for (i = 0; i < NumRows; i++){
              
                 var pageTotals = intVal(api.cell(i, 2).data());
                  pageTotal = pageTotal + pageTotals;
            }
               
            revenuetzss = pageTotal.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",");
            
              console.log(revenuetzss);

            for (i = 0; i < NumRows; i++){
              
                 var pageTotals2 = intVal(api.cell(i, 3).data());
        ...
        buttons: [
            {   extend: 'pdfHtml5',
                ...
                customize: function ( doc ) {
                  ...
                  doc.content[2].table.widths = [22, '*', 110, 110];
        ...
        fillColor: function (rowIndex, node, columnIndex) {
          return (rowIndex % 2 === 0) ? '#ffffff' : '#ffffff';
                    ...
                    doc.content.splice( 4, 0, {
                        margin: [ 0, 0, 0, 0],
                        table: {
                                widths: ['*', 80, 110, 109, 109],
                                body: [
                                [{text: 'TOTAL',  colSpan: 3, alignment: 'left'}, {},{}, {text: revenuetzss, alignment: 'right'}, {text: debttzs, alignment: 'right'}]
                              ]
                        }
                    });
                }
      ...
      "footerCallback": function ( row, data, start, end, display ) {
            ...
            var NumRows = $('#usdTable').DataTable().rows().count();
            var i, pageTotal = 0,pageTotalUsd = 0, pageTotal2 = 0, pageTotalUsd2 = 0;
