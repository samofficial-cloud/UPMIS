...
<table class="hover table table-striped  table-bordered" id="myTable2">
  <thead class="thead-dark">
        <tr>
          ...
          <th scope="col" style="width: 12%;"><center>Lease Start</center></th>
          <th scope="col" style="width: 12%;"><center>Lease End</center></th>
          <th scope="col" style="width: 8%;"><center>Currency</center></th>
        ...
        <tbody>
          ...
          <tr>
            ...
            <td scope="row" class="counterCell" style="text-align: center;">.</td>
              ...
              <td style="text-align: right;">{{$space->academic_season}}</td>
              <td style="text-align: right;">{{$space->vacation_season}}</td>
              @else
              <td style="text-align: right;">{{number_format($space->academic_season)}}</td>
              <td style="text-align: right;">{{number_format($space->vacation_season)}}</td>
              @endif
              @else
              @if(empty($space->amount))
                <td style="text-align: right;">{{$space->amount}}</td>
                <td style="text-align: right;">{{$space->amount}}</td>
              @else
               <td style="text-align: right;">{{number_format($space->amount)}}</td>
               <td style="text-align: right;">{{number_format($space->amount)}}</td>
              @endif
              @endif
...
<table class="hover table table-striped  table-bordered" id="myTable3">
                            ...
                            @foreach($invoices as $var)
                                <tr>
                                    ...
                                    <td><center>{{date("d/m/Y",strtotime($var->invoicing_period_start_date))}} - {{date("d/m/Y",strtotime($var->invoicing_period_end_date))}}</center></td>
...
function settitle(){
  return '\n\n <?php
    if($_GET['filter_date']=='true'){
      echo $_GET['space_id'].' Occupation History Report for the Duration from '.date("d/m/Y",strtotime($_GET['start_date'])).' to '.date("d/m/Y",strtotime($_GET['end_date']));
    }
    
     else{
         echo $_GET['space_id'].' Occupation History Report';
     }
  ?>';
}
...
function settitle2(){
  return '\n\n <?php
    if($_GET['filter_date']=='true'){
      echo $_GET['space_id'].' Debts Report for the Duration from '.date("d/m/Y",strtotime($_GET['start_date'])).' to '.date("d/m/Y",strtotime($_GET['end_date']));
    }
    
     else{
         echo $_GET['space_id'].' Debts Report';
     }
  ?>';
}
        ...
        buttons: [
            {   extend: 'pdfHtml5',
                ...
                customize: function ( doc ) {
                  ...
                  doc.content[2].table.widths = [22, '*', 80, 80, 60, 100, 100, 60];
...
var table = $('#myTable3').DataTable({ 
        ...
        buttons: [
            {   extend: 'pdfHtml5',
                ...
                orientation: 'Landscape',
                title: 'UNIVERSITY OF DAR ES SALAAM',
                messageTop: 'DIRECTORATE OF PLANNING, DEVELOPMENT AND INVESTIMENT'+settitle2(),
                pageSize: 'A4',
                exportOptions: {
                    columns: [ 0, 1, 2, 3, 4, 5]
                },
                ...
                customize: function ( doc ) {
                  ...
                  doc.content[2].table.widths = [22, '*', 80, 150, 80, 100];
                  var rowCount = doc.content[2].table.body.length;
                      for (i = 1; i < rowCount; i++) {
                         doc.content[2].table.body[i][0]=i+'.';
                      doc.content[2].table.body[i][1].alignment = 'left';
