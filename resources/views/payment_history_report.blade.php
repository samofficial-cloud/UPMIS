  ...
  <div class="container" style="max-width: 100%;">
    <center><h4>
     ...
     <br><br>
            ...
            @if($_GET['criteria']=='water')
                <span>Water bill</span>
            @else
            @endif


            @if($_GET['criteria']=='electricity')
            ...
            <span> for <b>{{$_GET['client_name']}}</b> </span>
          ...
          <div class="">
              ...
              <table style="font-size: 18px;width:40%" id="t_id">
                      ...
                      <tr>
                          ...
                          <td style="width: 20%">&nbsp;{{$_GET['client_name']}}</td>
                      </tr>
                      <tr>
                          <td>Location</td>
                          <td>&nbsp;{{$location}}</td>
...
<div class="row">
    ...
    <div class="col-12" style="overflow: auto;">
        ...
        <table class="hover table table-striped  table-bordered" style="width: 100%;" id="myTable">
            <thead class="thead-dark">
            <tr>
                <th scope="col"><center>S/N</center></th>
                <th scope="col" style="width: 16%;"><center>Invoicing Period</center></th>
                <th scope="col"><center>Academic Season Rent</center></th>
                <th scope="col" ><center>Long Vacation Season Rent</center></th>
                <th scope="col" ><center>Non Seasoned Rent</center></th>
                <th scope="col"><center>Escalation Rate(Academic Season)</center></th>
                <th scope="col"><center>Escalation Rate(Long Vacation Season)</center></th>
                <th scope="col"><center>Escalation Rate(Non Seasoned)</center></th>
                <th scope="col" ><center>Invoice No.</center></th>
                <th scope="col" ><center>Invoice Amount</center></th>
                <th scope="col" ><center>Date of Payment</center></th>
                <th scope="col" ><center>RT No.</center></th>
                <th scope="col" ><center>Paid Amount</center></th>
                <th scope="col" ><center>VAT(18%)</center></th>
                <th scope="col" ><center>Withholding Tax(10%)</center></th>
            ...
            @foreach($data as $var)
                <tr>
                    ...
                    <td>{{date("d/m/Y",strtotime($var->invoicing_period_start_date))}} to {{date("d/m/Y",strtotime($var->invoicing_period_end_date))}}</td>
                    @if($var->academic_dependence=='Yes')
                        <td>{{number_format($var->academic_season).' '.$var->currency}}</td>
                        <td>{{number_format($var->vacation_season).' '.$var->currency}}</td>
                    @else
                        <td>N/A</td>
                        <td>N/A</td>
                    @endif
                    ...
                    @if($var->academic_dependence=='Yes')
                    <td>{{$var->escalation_rate_academic}}%</td>
                    <td>{{$var->escalation_rate_vacation}}%</td>
                    @else
                        <td>N/A</td>
                        <td>N/A</td>
                    @endif
                    ...
                    <td>{{number_format(round(0.18*$var->amount_paid)).' '.$var->currency_payments}}</td>
                    <td>{{number_format(round(0.10*$var->amount_paid)).' '.$var->currency_payments}}</td>
                    <td>{{number_format($var->amount_not_paid).' '.$var->currency_payments}}</td>
                    <td>
                        @if($var->user_comments=='' || $var->user_comments==null)
                            N/A
                        @else
                            {{$var->user_comments}}
                        @endif
            ...
            $withhold_total_tzs=0;
            $withhold_total_usd=0;


            $invoice_amount_tzs=0;
            ...
            foreach ($data as $var){
                ...
                if($var->currency_invoice=='USD'){
                    $vat_total_usd+=round(0.18*$var->amount_paid);
                    $withhold_total_usd+=round(0.10*$var->amount_paid);
                    if($_GET['criteria']=='rent'){

                        $invoice_amount_usd+=$var->amount_to_be_paid;
                    }else{

                        $invoice_amount_usd+=$var->cumulative_amount;

                    }
                    $paid_amount_usd+=$var->amount_paid;
                    $balance_usd+=$var->amount_not_paid;
                }else{


                    $vat_total_tzs+=round(0.18*$var->amount_paid);
                    $withhold_total_tzs+=round(0.10*$var->amount_paid);
                    if($_GET['criteria']=='rent'){

                        $invoice_amount_tzs+=$var->amount_to_be_paid;
                    }else{

                        $invoice_amount_tzs+=$var->cumulative_amount;

                    }
                    $paid_amount_tzs+=$var->amount_paid;
                    $balance_tzs+=$var->amount_not_paid;
            ...
            <tr>
                ...
                <td>Total TZS= {{number_format($invoice_amount_tzs)}}</td>
                <td></td>
                <td></td>
                <td>Total TZS= {{number_format($paid_amount_tzs)}}</td>
                <td>Total TZS= {{number_format($vat_total_tzs)}}</td>
                <td>Total TZS= {{number_format($withhold_total_tzs)}}</td>
                <td>Total TZS= {{number_format($balance_tzs)}}</td>
                <td></td>
            ...
            <tr>
                ...
                <td>Total USD= {{number_format($invoice_amount_usd)}}</td>
                <td></td>
                <td></td>
                <td>Total USD= {{number_format($paid_amount_usd)}}</td>
                <td>Total USD= {{number_format($vat_total_usd)}}</td>
                <td>Total USD= {{number_format($withhold_total_usd)}}</td>
                <td>Total USD= {{number_format($balance_usd)}}</td>
                <td></td>
...
<script>
    ...
    <?php
     ...
     $title='';
        ...
        $title.='Payment History Report for '.$_GET['client_name'];
    ...
    var table = $('#myTable').DataTable( {
        dom: '<"top"f><"top"<"pull-right" B>>rt<"bottom"pi>',
        "pageLength": 100,
        buttons: [
            {   extend: 'pdfHtml5',
                filename:title,
                ...
                customize: function ( doc ) {
                    ...
                    doc.content[2].table.widths=[15, 38, 38, 38, 38, 38, 38, 38,38,38,38,38,38,38,38,38,38];
